...
var fs, path, textTransformers, MustacheComb;
...
MustacheComb.prototype.mustacheIncluder = function mustacheIncluder(text) {
    var out,
        relPath;
    // the text of the include tag is a url, possibly surrounded with whitespace.
    relPath = text.trim();
...
MustacheComb.prototype.buildArticles = function buildArticles() {
    var my = this;
    this.articles.forEach(function (article) {
        console.log('article write attempt', article.title);
        var html = my.mainMustache(article);
        fs.writeFileSync(path.join(my.outdir, article.out), html, 'utf8');
        console.log('article created', my.outdir + article.out);
    });
};
