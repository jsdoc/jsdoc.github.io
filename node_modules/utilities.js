...
directory.filterList = function filterList(dir, filt) {
    ...
    try {
        ...
        console.log('There was an error accessing files.'.warn);
    }
    return filesArray;
};
    ...
    try {
        fs.readdirSync(dir).forEach(function (file) {
            if (/\.html$/.test(file)) {
                ...
                console.log('File removed'.info + ' : ', remove.data);
            }
        });
    } catch(err) {
        console.log('There was an error when purging the HTML files.'.warn);
    }
};
